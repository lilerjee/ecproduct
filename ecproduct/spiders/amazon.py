...
For books, the ASIN is the same as the ISBN number, but for all other products a new ASIN is created when the item is uploaded to our catalogue. 
You will find an item's ASIN on the product detail page alongside further details relating to the item,
which may include information such as size, number of pages (if it's a book) or number of discs (if it's a CD).
ASINs can be used to search for items in our catalogue. If you know the ASIN or ISBN of the item you are looking for,
simply type it into the search box (which can be found near the top of most pages), hit the "Go" button and,
if the item is listed in our catalogue, it will appear in your search results.
...
class AmazonSpider(EcproductSpider):
    name = 'amazon'
    ...
    def _get_product_info(self, loader, response):
       ...
       # product url '^https?://([A-Za-z0-9\-]+\.)+vvic\.com/item/(\d+)$')
        # loader.add_value('product_id', self.url.strip(), Compose(lambda v: v[-1]), re='/dp/(\w+)/')
        loader.add_value('product_id', re.findall(r'"currentAsin"\s*:\s*"(\w+)"', response.text))
        loader.add_css('product_name', 'span#productTitle::text', MapCompose(lambda v: v.strip()))
        loader.add_css('product_sale_price', 'span#priceblock_ourprice::text', re=r'\$([\d\.]+)')
        loader.add_css('product_original_price', 'span#priceblock_ourprice::text', re=r'\$([\d\.]+)')
    ...
    def parse_from_index_page(self, response):
        ...
        if data_type in ['market', 'product']:
            ...
            global_markets = re.findall(
                    r"{panggeFlag=(\d+),\s*code=(\w+),\s*rankFlag=(\d+),\s*name=(\w+),\s*id=(\d+),\s*strengthFlag=\d+}", 
                    global_markets_str)
            next_url_list = []
            next_code_list = []
            next_addr_list = []
