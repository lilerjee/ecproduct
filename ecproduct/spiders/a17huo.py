...
class A17huoSpider(EcproductSpider):
    name = 'a17huo'
    ...
    def parse_from_category_page(self, response):
        ...
        parent_category_id = (
            response.meta['parent_category_id']
            if 'parent_category_id' in response.meta.get else 0)

        if data_type == 'category':
            category_url = (
                'http://api.open.haoduoyifu.com/router/17huo.category.get?' +
                'parent_cid=%s' % parent_category_id)
...
# 17huo认证工具类，包括请求17huo open api的token，刷新token，将token保存到redis中
class Authorize17HuoUtils:
    CLIENT_ID = '17324314'
    CLIENT_SECRET = 'm70j8A5UdzJqhzhMS5PPbl92x0TbcxNYoQN6VxHb'
    USER = '18776666617'
    PASSWORD = 'litb2018@'
    ACCESS_TOKEN_REQUEST_URL = (
        'http://api.open.haoduoyifu.com/router/oauth/token')

    def __init__(self):
        pass

    def get_init_access_token(self):
        request = self.create_token_request()
        data = {
        ...
        token_msg.token_type = response_json.get('token_type')
        token_msg.expires_in = response_json.get('expires_in')
        token_msg.access_token = response_json.get('access_token')
        token_msg.refresh_token = response_json.get('refresh_token')

        self.save_token(token_msg)
        return token_msg.access_token
    ...
    def save_token(self, token_msg):
        if token_msg is None:
            logging.INFO('can not fetch token')
            return
        data = {'token_type': token_msg.token_type, 'expires_in': token_msg.expires_in,
                'access_token': token_msg.access_token, 'refresh_token': token_msg.refresh_token}
        # 17huo token过期时间30天，设置本地过期时间为29天
        # 因为17活所有异常返回的状态都是一样的，不好区分，新的token可无限制获取，
        # 故不采用refresh token的方式，过期后直接重新获取
        RedisService.set(key='17huo', value=json.dumps(data, encoding='utf-8'), expire_time=29 * 24 * 60 * 60)

    def get_access_token(self, key=''):
        access_string = RedisService.get(key)
        access_token = self.get_init_access_token() if access_string is None else json.loads(access_string).get(
            'access_token')
        return access_token
