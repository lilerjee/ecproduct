...
from ecproduct.spiders.tmall import TmallSpider
...
class TaobaoSpider(EcproductSpider):
    name = 'taobao'
    ...
    def parse_from_market_with_product_page(self, response):
        ...
        if data_type == 'market':
            ...
            # redirect to tmall
            if 'tmall.com' in response.url:     # the url jumped to other platform shop url
                tmall = TmallSpider(response.url, 'market',  data_type)
                response.meta['platform_code'] = tmall.platform_code
                response.meta['original_url'] = response.url
                items = tmall.parse_from_market_with_product_page(response)
                # yield from item
                for item in items:
                    # generate record which will jump
                    loader.add_value('platform_code', platform_code)
                    find_shopid = re.findall(r'https?://(\w+)\.taobao\.com', original_url)
                    loader.add_value('market_id', find_shopid[0] if find_shopid else None)
                    loader.add_value('market_url', original_url)
                    loader.add_value('market_type', 'shop')
                    loader.add_value('related_type', 'jump')
                    loader.add_value('market_status', '1')
                    loader.add_value('related_platform_code', item['platform_code'])
                    loader.add_value('related_market_id', item['market_id'])
                    loader.add_value('related_market_url', item['market_url'])
                    loader.add_value('created_time',  datetime.datetime.strftime(datetime.datetime.today(),  '%Y-%m-%d %H:%M:%S'))
                    yield loader.load_item()

                    # generate record which will be jumped
                    item['related_type'] = ['is jumped']
                    item['related_platform_code'] = [platform_code]
                    item['related_market_id'] = [find_shopid[0] if find_shopid else None]
                    item['related_market_url'] = [original_url]
                    yield item
                return 
            ...
            loader.add_value('market_status', '1')
            loader.add_css('market_name', 
                    'div.shop-summary p.summary-line span.line-left span.shop-name a::text', 
                    lambda v: v[0].strip() if v else v)
            loader.add_css('market_name', 
                    'div.shop-summary div.shop-name div.shop-name-wrap a::text', 
                    lambda v: v[0].strip() if v else v)
            loader.add_css('market_name', 
                    'div#shop-head div.clear div#shop-info div.shop-info-simple > a::text', 
                    lambda v: v[0].strip() if v else v)

            # get score info 
            for s in response.css('div.summary-popup div.qualifications-dsr div.shop-rate ul li'):
                loader.add_value('market_attr_type', 'score')
                loader.add_value('market_attr_name', s.xpath('text()').extract_first().strip())
                loader.add_value('market_attr_value', s.xpath('em/text()').extract_first().strip())
